...
export class MessageRouter {
    ...
    async dispatch(event, context) {
        ...
        const panel = context.panelDeVentana?.(event.source) || context.panelAlFrente?.() || null;
        if (!panel && !data.type.startsWith('producciontv:open-color-picker') && !data.type.startsWith('producciontv:close-color-picker')) return false;
...
export function createShellMessageRouter(bridge) {
    ...
    // Apertura del Selector Nativo de Colores macOS (NSColorPanel)
    router.register('producciontv:open-color-picker', (data, panel, ctx) => {
        let absLeft = data.elemLeft || 0;
        let absBottom = data.elemBottom || 0;
        if (panel?.el) {
            const iframeRect = panel.el.getBoundingClientRect();
            absLeft += iframeRect.left;
            absBottom += iframeRect.top;
        }
        ctx.setActiveColorTag?.(data.tag || 'default');
        const openPicker = ctx.OpenNativeColorPicker || globalThis.go?.main?.App?.OpenNativeColorPicker;
        if (openPicker) {
            openPicker(absLeft, absBottom, data.initialHex || '#1D6FD1');
        }
    });

    // Cierre del Selector Nativo de Colores macOS
    router.register('producciontv:close-color-picker', (data, panel, ctx) => {
        const closePicker = ctx.CloseNativeColorPicker || globalThis.go?.main?.App?.CloseNativeColorPicker;
        if (closePicker) {
            closePicker();
        }
    });
