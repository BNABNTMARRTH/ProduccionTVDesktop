...
export function EscaletaEditor({ cfg, setCfg, rows, editable, fuentes = [] }) {
  ...
  const cols = narr
    ...
    : "30px 46px minmax(120px,1.1fr) minmax(150px,1.4fr) minmax(110px,1fr) minmax(120px,1fr) minmax(120px,1fr) 50px 50px 58px 66px";
  const heads = narr
    ? ["SEC", "ESC", "ENCABEZADO", "ACCIÓN PRINCIPAL", "FUNCIÓN NARRATIVA", "PERSONAJES", "CAMBIO", "DUR", ""]
    : ["#", "BLOQUE", "SEGMENTO", "OBJETIVO", "PARTICIPANTES", "RECURSOS", "FUENTE", "IN", "OUT", "DUR", ""];
      ...
      {rows.map((r, i) => (
        ...
        <div className="grid items-center gap-1" style={{ gridTemplateColumns: cols }}>
          {narr ? (<>
            ...
            <span className="text-center text-xs font-bold" style={{ color: "var(--ui-tinta-media)" }}>{r.idx}</span>
            ...
            {/* LA FUENTE AL AIRE VIVE AQUÍ DESDE EL 1-SEP-2026. El comentario de
                arriba decía que no, y tenía razón mientras hubo una segunda
                pantalla (la pestaña de Tiempos) donde ponerla; esa pantalla era
                una copia de ésta y se quitó, así que la fuente se vino con
                ella. Sigue siendo distinto del rundown: aquí es "de dónde sale
                este bloque", allá es cada cue técnico dentro del bloque. */}
            <select className={ei} style={eiS} disabled={!editable} value={r.fuente ?? ""} onChange={(e) => up(r.id, { fuente: e.target.value })}>
              <option value="">— sin fuente —</option>
              {fuentes.map((f) => <option key={f.id} value={f.id}>{f.nombre}</option>)}
            </select>
            <span className="text-center text-[11px]" style={{ color: "var(--ui-tinta-media)" }}>{fmt(r.tin)}</span>
            <span className="text-center text-[11px]" style={{ color: "var(--ui-tinta-media)" }}>{fmt(r.tout)}</span>
          </>)}
        ...
        {/* La nota del segmento, en su propio renglón: metida en la rejilla
            dejaría cada columna sin aire. También venía de la pestaña de
            Tiempos. En narrativo la nota es el guion del bloque y se escribe en
            Guion técnico, así que aquí solo va en vivo. */}
        {!narr && (
          <textarea className={ei} style={{ ...eiS, resize: "vertical", minHeight: 28, lineHeight: 1.4 }} rows={r.nota ? 2 : 1}
            disabled={!editable} placeholder="Notas del segmento: guion del conductor, cue de audio, aviso de efectos…"
            value={r.nota ?? ""} onChange={(e) => up(r.id, { nota: e.target.value })} />
        )}
        </div>
      ))}
      {!rows.length && <p className="text-sm" style={{ color: "var(--ui-tinta-media)" }}>La escaleta está vacía.{editable ? " Agrega un segmento o usa el asistente." : ""}</p>}
      {editable && (
        <button onClick={add} className={`${btn} self-start border`} style={{ borderColor: "var(--ui-linea)", color: "var(--gel-t)" }}>
